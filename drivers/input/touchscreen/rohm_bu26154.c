...
static inline void calibrate(int *x, int *y)
{
	int cal_x, cal_y;
	int raw_x = *x, raw_y = *y;
	...
	cal_x = calibration[0] * raw_x +
		calibration[1] * raw_y + calibration[2];
	cal_x /= calibration[6];
	if (cal_x < 0)
		cal_x = 0;

	cal_y = calibration[3] * raw_x +
		calibration[4] * raw_y + calibration[5];
	cal_y /= calibration[6];
	if (cal_y < 0)
		cal_y = 0;
